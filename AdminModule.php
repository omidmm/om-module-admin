...
use omidmm\admin\assets\LiveAsset;
...
class AdminModule extends \yii\base\Module implements BootstrapInterface
{
    ...
    public function init()
    {
        ...
        define('IS_ROOT',  !Yii::$app->user->isGuest && Yii::$app->user->can('Root'));
        define('LIVE_EDIT', !Yii::$app->user->isGuest && Yii::$app->session->get('progsoft_live_edit'));
    }


    public function bootstrap($app)
    {
        Yii::setAlias('progsoft', '@common/progsoft');

        if(!$app->user->isGuest && Yii::$app->user->can('Root') && strpos($app->request->pathInfo, 'admin') === false) {
    ...
    public function renderToolbar()
    {
        ...
        echo $view->render('@progsoft/views/layouts/frontend-toolbar.php');
    }
    ...
    public function getInstalled()
    {
        if($this->_installed === null) {
            try {
                $this->_installed = Yii::$app->db->createCommand("SHOW TABLES LIKE 'progsoft_%'")->query()->count() > 0 ? true : false;
            } catch (\Exception $e) {
