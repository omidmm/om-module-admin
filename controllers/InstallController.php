...
use omidmm\admin\helpers\WebConsole;
use omidmm\admin\models\InstallForm;
...
class InstallController extends \yii\web\Controller
{
    ...
    public function actionIndex()
    {
        if(!$this->checkDbConnection()){
            ...
            return $this->showError(Yii::t('modules/install', 'Cannot connect to database. Please configure `{0}`.', $configFile));
        }
        if($this->module->installed){
            return $this->showError(Yii::t('modules/install', 'progsoftCMS is already installed. If you want to reinstall progsoftCMS, please drop all tables with prefix `progsoft_` from your database manually.'));
        }
    ...
    {
        Yii::$app->i18n->translations['progsoft/modules/install'] = [
            'class' => 'yii\i18n\PhpMessageSource',
            'sourceLanguage' => 'en-US',
            'basePath' => '@progsoft/messages',
            'fileMap' => [
                'progsoft/modules/install' => 'install.php',
            ]
        ];
    }
    ...
    private function insertSettings($installForm)
    {
        ...
        $db->createCommand()->insert(Setting::tableName(), [
            ...
            'title' => Yii::t('modules/install', 'ReCaptcha key'),
        ...
        $db->createCommand()->insert(Setting::tableName(), [
            'name' => 'root_password',
            'value' => $root_password,
            'title' => Yii::t('modules/install', 'Root password'),
            'visibility' => Setting::VISIBLE_ROOT
        ])->execute();
        ...
        $db->createCommand()->insert(Setting::tableName(), [
            'name' => 'auth_time',
            'value' => 86400,
            'title' => Yii::t('modules/install', 'Auth time'),
            'visibility' => Setting::VISIBLE_ROOT
        ])->execute();
        ...
        $db->createCommand()->insert(Setting::tableName(), [
            ...
            'title' => Yii::t('modules/install', 'Robot E-mail'),
            'visibility' => Setting::VISIBLE_ROOT
        ])->execute();
        ...
        $db->createCommand()->insert(Setting::tableName(), [
            ...
            'title' => Yii::t('modules/install', 'Admin E-mail'),
        ...
        $db->createCommand()->insert(Setting::tableName(), [
            ...
            'title' => Yii::t('modules/install', 'ReCaptcha secret'),
            'visibility' => Setting::VISIBLE_ROOT
        ])->execute();
        ...
        $db->createCommand()->insert(Setting::tableName(), [
            ...
            'value' => 'top',
            'title' => Yii::t('modules/install', 'Frontend toolbar position').' ("top" or "bottom")',
            'visibility' => Setting::VISIBLE_ROOT
        ])->execute();
    }
    ...
    private function installModules()
    {
        ...
        foreach(glob(Yii::getAlias('@progsoft'). DIRECTORY_SEPARATOR .'modules/*') as $module)
        {
            $moduleName = basename($module);
            $moduleClass = 'omidmm\admin\modules\\' . $moduleName . '\\' . ucfirst($moduleName) . 'Module';
